...
}else{if(k.indexOf("a")===0&&i==12){i=0;}}}else{if(i<12&&(i<7||i<n.getHours())){i+=12;}}n.setHours(i,l,0);return j[0];}else{return false;}}}}},g=function(n,m,k){var j;
if(j=n.match(d.monthDay)){m.setMonth(e.changeMonth(j[1]),j[2]);return j[0];}else{if(j=n.match(d.dayMonth)){m.setMonth(e.changeMonth(j[2]),j[1]);return j[0];
}else{if(j=n.match(d.weekdays)){switch(j[2].substr(0,3)){case"sun":e.changeDay(m,0,j[1]);return j[0];case"mon":e.changeDay(m,1,j[1]);return j[0];case"tue":e.changeDay(m,2,j[1]);
return j[0];case"wed":e.changeDay(m,3,j[1]);return j[0];case"thu":e.changeDay(m,4,j[1]);return j[0];case"fri":e.changeDay(m,5,j[1]);return j[0];case"sat":e.changeDay(m,6,j[1]);
return j[0];default:return false;}}else{if(j=n.match(d.inRelativeDateFromRelativeDate)){if(e.relativeDateMatcher(j[3],m)&&e.inRelativeDateMatcher(j[1],j[2],m)){return j[0];
...
}}else{l.setFullYear(l.getFullYear()+1);}}}}},e={relativeDateMatcher:function(j,k){var i=b();switch(j){case"next week":k.setMonth(i.getMonth(),i.getDate()+7);
return true;case"next month":k.setMonth(k.getMonth()+1);return true;case"tom":k.setMonth(i.getMonth(),i.getDate()+1);return true;case"tomorrow":k.setMonth(i.getMonth(),i.getDate()+1);
return true;case"day after tomorrow":k.setMonth(i.getMonth(),i.getDate()+2);return true;case"day after tom":k.setMonth(i.getMonth(),i.getDate()+2);return true;
case"today":k.setMonth(i.getMonth(),i.getDate());return true;case"tod":k.setMonth(i.getMonth(),i.getDate());return true;default:return false;}},inRelativeDateMatcher:function(i,k,j){if(isNaN(i)){i=1;
}else{i=parseInt(i);}switch(k){case"day":j.setDate(j.getDate()+i);return true;case"week":j.setDate(j.getDate()+i*7);return true;case"month":j.setMonth(j.getMonth()+i);
return true;default:return false;}},changeMonth:function(i){return this.monthToInt[i.substr(0,3)];},changeDay:function(l,j,i){var k=7-l.getDay()+j;if(k>7&&!i){k-=7;
}l.setDate(l.getDate()+k);},escapeRegExp:function(i){return i.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g,"\\$&");},isSameDay:function(j,i){return j.getMonth()===i.getMonth()&&j.getDate()===i.getDate()&&j.getFullYear()===i.getFullYear();
