 ...
 * Version 1.0
 */
var Sherlock=(function(){var b={rangeSplitters:/(\bto\b|\-|\b(?:un)?till?\b|\bthrough|and\b)/g,months:"\\b(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\\b",days:"\\b(?:(?:(?:on )?the )(?=\\d\\d?(?:st|nd|rd|th)))?(0?[1-9]|[1-2]\\d|3[0-1])(?:st|nd|rd|th)?(?:,|\\b)",shortForm:/\b(0?[1-9]|1[0-2])(?:\/|\.)(0?[1-9]|[1-2]\d|3[0-1])/,weekdays:/(next (?:week (?:on )?)?)?\b(sun|mon|tue(?:s)?|wed(?:nes)?|thurs|fri|sat(?:ur)?)(?:day)?\b/,relativeDate:/\b(next (?:week|month)|tom(?:orrow)?|tod(?:ay)?|day after tom(?:orrow)?)\b/,inRelativeDate:/\b(\d{1,2}|a) (day|week|month)s?\b/,inRelativeTime:/\b(\d{1,2}|a|an) (hour|min(?:ute)?)s?\b/,midtime:/(?:@ ?)?\b(?:at )?(noon|midnight)\b/,militaryTime:/\b([0-2]\d):?([0-5]\d)\b/,explicitTime:/(?:@ ?)?\b(?:at |from )?(0?[1-9]|1[0-2])(?::([0-5]\d))? ?([ap]\.?m?\.?)?\b/,hoursOnly:/^(0?[1-9]|1[0-2])$/,fillerWords:/ (from|is|at|on|for|in|(?:un)?till?)\b/},e=function(l,j,i){var h={},f=false,g=false,k=c.strToNum(l);
...
if(n.getHours()<=f.getHours()&&n.getMinutes()<=f.getMinutes()){n.setDate(n.getDate()-1);o.setDate(o.getDate()-1);}if(n<f&&o>f){g.startDate=n;i.setDate(o.getDate());
}}}return l[0];},d=function(k,h,g){var i,j,l;if(i=k.match(b.monthDay)){j=c.changeMonth(i[1]);l=i[2];}else{if(i=k.match(b.dayMonth)){j=c.changeMonth(i[2]);
...
return i[0];case"sat":c.changeDay(h,6,i[1]);return i[0];default:return false;}}else{if(i=k.match(b.relativeDate)){var f=new Date();switch(i[1]){case"next week":h.setDate(h.getDate()+7);
return i[0];case"next month":h.setMonth(h.getMonth()+1);return i[0];case"tom":h.setMonth(f.getMonth());h.setDate(f.getDate()+1);return i[0];case"tomorrow":h.setMonth(f.getMonth());
h.setDate(f.getDate()+1);return i[0];case"day after tomorrow":h.setMonth(f.getMonth());h.setDate(f.getDate()+2);return i[0];case"day after tom":h.setMonth(f.getMonth());
h.setDate(f.getDate()+2);return i[0];case"today":h.setMonth(f.getMonth());h.setDate(f.getDate());return i[0];case"tod":h.setMonth(f.getMonth());h.setDate(f.getDate());
return i[0];default:return false;}}else{if(i=k.match(b.inRelativeDate)){if(isNaN(i[1])){i[1]=1;}switch(i[2]){case"day":h.setDate(h.getDate()+parseInt(i[1]));
...
}}else{if(i=k.match(b.shortForm)){j=i[1]-1;l=i[2];}else{if(i=k.match(new RegExp(b.days,"g"))){i=i.sort(function(p,o){return o.length-p.length;})[0];if((i.indexOf("on")!==0&&!(i.indexOf("the")===0&&i.indexOf(",",i.length-1)!==-1)&&k.indexOf(i,k.length-i.length-1)===-1)||(!(g&&g.isAllDay)&&i.length<=2&&i.match(b.hoursOnly))){return false;
